 ...
 * slots on a plain script. The rest of the table (new_coroutine, resume,
 * cancel) speaks about a live coroutine, which cannot exist while no
 * scheduler is registered — those slots stay NULL, so a caller that
 * reaches them is a bug and dies loudly instead of getting a polite
 * exception. */
...
ZEND_API zend_async_cancel_t zend_async_cancel_fn = NULL;

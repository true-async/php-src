...
static int curl_seek(void *clientp, curl_off_t offset, int origin)
{
	...
	/* In async mode libcurl drives the callback from the reactor, where there is
	 * no PHP frame to catch a throw. Hand the exception to the awaiting coroutine
	 * and tell libcurl the seek failed. */
	if (EG(exception) && ch->async_event != NULL) {
		curl_async_event_t *curl_event = (curl_async_event_t *) ch->async_event;
		GC_ADDREF(EG(exception));
		curl_async_event_set_callback_exception(curl_event, EG(exception));

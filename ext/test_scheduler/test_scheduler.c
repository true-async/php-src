 ...
 * a binary carrying it must still be able to test other providers (the PHP
 * SchedulerHook bridge). Tests opt in with test_scheduler.enable=1. */
PHP_INI_BEGIN()
...
static HashTable *ts_coroutine_object_gc(zend_object *object, zval **table, int *num)
{
	 ...
	 * enable any collection either. Verified empirically: the walk produced

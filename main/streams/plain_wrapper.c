...
static zend_always_inline bool php_stdiop_unpin_after_suspend(php_stream *stream, php_stdio_stream_data *data)
{
	...
	if (--data->ref_count == 0 && was_closed) {
		...
		/* php_stream_free left the read buffer to the last holder. */
...
static int php_stdiop_sync(php_stream *stream, bool dataonly)
{
	...
	if (data->async_io != NULL) {
		...
		if (!req->completed) {
			zend_coroutine_t *const coroutine = ZEND_ASYNC_CURRENT_COROUTINE;
			/* The io event is shared, so another request's completion wakes
			 * us; req->result of an unfinished fsync reads as success. Pin
			 * stream/data: a close from another coroutine would free them. */
			data->ref_count++;
			do {
				ZEND_ASYNC_WAKER_NEW(coroutine);
				zend_async_resume_when(coroutine, &data->async_io->event, false,
...
static int php_stdiop_cast(php_stream *stream, int castas, void **ret)
{
	...
	switch (castas)	{
		...
		case PHP_STREAM_AS_FD_FOR_COPY:
			...
			/* A pipe managed by the reactor is kept non-blocking, so a raw splice()
			 * or read() on it fails with EAGAIN instead of waiting. Refuse the
			 * cast and let the copy go through the stream ops, which suspend the
			 * coroutine until data arrives. Regular files are always ready, so
			 * they keep the descriptor-level copy. */
			if (data->is_pipe && data->async_io != NULL && !ZEND_ASYNC_IS_OFF && data->is_blocked) {
				return FAILURE;
			}
			PHP_STDIOP_GET_FD(fd, data);
			if (SOCK_ERR == fd) {
